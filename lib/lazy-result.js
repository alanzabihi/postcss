...
class LazyResult {
  ...
  sync() {
    ...
    if (this.hasListener) {
      let root = this.result.root
      let hd = Node.highestDirty
      // First full walk of the entire tree
      root[isClean] = true
      root[hd] = undefined
      this.walkSync(root)
      // Subsequent re-walks: only re-walk from the highest dirty ancestor
      // instead of re-walking the entire root tree
      while (!root[isClean]) {
        let target = root[hd] || root
        root[isClean] = true
        root[hd] = undefined
        if (target === root) {
          this.walkSync(root)
        } else {
          this.walkSync(target)
        }
      }
      root[hd] = undefined
      if (this.listeners.OnceExit) {
