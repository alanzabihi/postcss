...
function getEvents(node) {
  let key = false
  let type = TYPE_ID_TO_CLASS_NAME[node._typeId]
  if (node._typeId === TYPE_DECL) {
    key = node.prop.toLowerCase()
  } else if (node._typeId === TYPE_ATRULE) {
...
function toStack(node) {
  let events
  if (node._typeId === TYPE_DOCUMENT) {
    events = ['Document', CHILDREN, 'DocumentExit']
  } else if (node._typeId === TYPE_ROOT) {
