...
class Parser {
  ...
  decl(tokens, customProperty) {
    ...
    while (tokens[ti][0] !== 'word') {
      if (tokens.length - ti === 1) this.unknownWord(tokens)
      node.raws.before += tokens[ti][1]
      ti++
    }
    node.source.start = this.getPosition(tokens[ti][2])

    node.prop = ''
    while (ti < tokens.length) {
      let type = tokens[ti][0]
      if (type === ':' || type === 'space' || type === 'comment') {
        break
      }
      node.prop += tokens[ti][1]
      ti++
    }
    ...
    let fsStart = ti
    while (ti < tokens.length) {
      let next = tokens[ti][0]
      if (next !== 'space' && next !== 'comment') break
      ti++
    }
    let firstSpaces
    if (ti > fsStart) {
      firstSpaces = tokens.slice(fsStart, ti)
    } else {
      firstSpaces = []
    }

    if (ti > 0) tokens.splice(0, ti)
  ...
  spacesAndCommentsFromEnd(tokens) {
    ...
    let boundary = i + 1
    if (boundary === tokens.length) return ''
    let spaces = ''
    for (let j = boundary; j < tokens.length; j++) {
      spaces += tokens[j][1]
    }
    tokens.length = boundary
    return spaces
  }
